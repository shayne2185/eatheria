// EATHERIA – Zen swap + push-up game.js
...
startBtn.onclick = () => {
  ...
  resize();
  initGrid();
  resolveAllMatches(); // odstráň prípadné autá matchy na štarte
...
canvas.addEventListener("pointerup", e => {
  ...
  handleSwipe(dx, dy, pointerSwipeStart.x, pointerSwipeStart.y);
...
canvas.addEventListener("touchend", e => {
  ...
  if (e.touches.length === 0 && singleTouchStart) {
    ...
    handleSwipe(dx, dy, singleTouchStart.x, singleTouchStart.y);
...
// ===== SWIPE HANDLER =====
function handleSwipe(dx, dy, startX, startY) {
  if (!canAcceptInput()) return;
  if (Math.abs(dx) < 15 && Math.abs(dy) < 15) return;

  const info = getBoardInfo(startX, startY);
  ...
  // SWAP ikoniek (časť 3)
  if (inBoard) {
    attemptSwap(dx, dy, row, col);
  }
}
// ===== SWAP LOGIKA =====
function attemptSwap(dx, dy, row, col) {
  // ochrana: mimo grid / mimo rozsah
  if (row < 0 || row >= ROWS || col < 0 || col >= COLS) return;

  // ak sa skoro nič nepohlo, ignoruj
  if (Math.abs(dx) < 15 && Math.abs(dy) < 15) return;

  let targetRow = row;
  let targetCol = col;

  // rozhodni smer podľa väčšej absolútnej zmeny
  if (Math.abs(dx) > Math.abs(dy)) {
    // horizontálny smer
    if (dx > 0) {
      targetCol = col + 1; // doprava
    } else {
      targetCol = col - 1; // doľava
    }
  } else {
    // vertikálny smer
    if (dy > 0) {
      targetRow = row + 1; // dole
    } else {
      targetRow = row - 1; // hore
    }
  }

  // blok: horný riadok nesmie ísť hore mimo grid
  if (row === 0 && targetRow < 0) {
    return;
  }

  // ochrana: cieľ mimo gridu
  if (
  ...
  // swap sa vykoná vždy (aj bez matchu)
  performSwap(row, col, targetRow, targetCol);

  // po swape vyhodnoť matchy
  resolveAllMatches();
}
...
function applyMatches(matches) {
  // vytvor miznúce efekty a vymaž ikonky
...
function collapseAndRefill() {
  ...
  // refill – len spodné AUTO_FILL_ROWS riadky, cluster-friendly
...
// ===== PUSH-UP =====
function pushUp() {
  // posuň všetko o 1 riadok hore
  for (let r = 0; r < ROWS - 1; r++) {
    for (let c = 0; c < COLS; c++) {
      grid[r][c] = grid[r + 1][c];
    }
  }
  // nový spodný riadok – cluster-friendly
  const below = ROWS >= 2 ? grid[ROWS - 1] : null;
  grid[ROWS - 1] = generateClusteredRow(below);

  // po push-upe vyhodnoť matchy
  resolveAllMatches();
}
