...
startBtn.onclick = () => {
  ...
  resize();
  initGrid();
  needsResolve = true; // vyčisti príp. matchy na štarte
...
function loop(timestamp) {
  ...
  // auto-resolve po každej zmene gridu
  if (needsResolve) {
    resolveAllMatches();
    needsResolve = false;
  }
...
function drawVanishOverlay() {
  ...
  for (const eff of vanishEffects) {
    ...
    const iconSize = iconBaseSize * (1 - 0.3 * progress);
    const ix = tileX + (tileSize - iconSize) / 2;
    const iy = tileY + (tileSize - iconSize) / 2;

    // jemná biela para okolo
    ctx.save();
    ctx.globalAlpha = alpha * 0.6;
    const cloudRadius = tileSize * (0.55 + 0.3 * progress);
    const cx = tileX + tileSize / 2;
    const cy = tileY + tileSize / 2;

    const grad = ctx.createRadialGradient(
      cx, cy, cloudRadius * 0.2,
      cx, cy, cloudRadius
    );
    ...
    // ikonka, ktorá sa zmenšuje a mizne
...
canvas.addEventListener("touchmove", e => {
  ...
  // 2-prstový swipe hore = push-up, ale len ak začal v hint zóne pod gridom
...
// ===== SINGLE SWIPE – SWAP alebo PUSH-UP (1 prst) =====
function handleSingleSwipe(dx, dy, startX, startY) {
  ...
  // 1-prstový push-up – len ak swipe začal v hint zóne pod gridom
  if (startY >= hintTop && startY <= hintBottom && dy < -25 && Math.abs(dy) > Math.abs(dx)) {
    doPushUp();
    return;
  }

  // SWAP – kdekoľvek vnútri gridu (aj v hornom riadku)
...
function swap(r1, c1, r2, c2) {
  ...
  // grid sa zmenil → skús auto-match
  needsResolve = true;
}
...
function pushUp() {
  ...
  needsResolve = true; // po push-up tiež skontroluj match‑3+
}
