...
let lineAnim = null; // { type: 'row'|'col', index, dir, t, duration, offset }
...
startBtn.onclick = () => {
  ...
  resize();
  initGrid();
  resolveAllMatches(); // vyčisti eventuálne auto-matchy na štarte
...
function drawGrid() {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      if (lineAnim && lineAnim.type === "row" && lineAnim.index === r) {
        dx = lineAnim.offset;
      } else if (lineAnim && lineAnim.type === "col" && lineAnim.index === c) {
        dy = lineAnim.offset;
      }

      const baseX = offsetX + c * SIZE + dx;
      const baseY = offsetY + r * SIZE + dy + pushOffset;
...
// ===== ROW/COL ANIM =====
function startLineAnim(type, index, dir) {
  lineAnim = {
    type, // 'row' | 'col'
    index,
    dir,  // -1 alebo +1
...
function updateLineAnim(dt) {
  if (!lineAnim) return;
  lineAnim.t += dt;
  const p = Math.min(lineAnim.t / lineAnim.duration, 1);
  const eased = 1 - Math.pow(1 - p, 2);
  const target = lineAnim.dir * SIZE;
  lineAnim.offset = target * (1 - eased);
  if (p >= 1) {
    lineAnim = null;
    // keď animácia skončí, vyhodnoť matchy
    resolveAllMatches();
  }
}
...
function canAcceptInput() {
  return !lineAnim && !pushAnim.active;
}
...
function getBoardCoords(clientX, clientY) {
  ...
  return { inBoard, col, row, yCanvas: y, boardBottom };
}
...
canvas.addEventListener("touchmove", e => {
  ...
  if (e.touches.length === 2 && multiTouchStart) {
    ...
    const boardInfo = getBoardCoords(
      (multiTouchStart[0].x + multiTouchStart[1].x) / 2,
      startAvgY
    );
    const hintTop = boardInfo.boardBottom + 10;
...
// ===== SINGLE SWIPE – ROW/COL SHIFT + PUSH-UP =====
function handleSingleSwipe(dx, dy, startClientX, startClientY) {
  if (!canAcceptInput()) return;
  if (Math.abs(dx) < 15 && Math.abs(dy) < 15) return;

  const info = getBoardCoords(startClientX, startClientY);
  const { inBoard, col, row, yCanvas, boardBottom } = info;

  const hintTop = boardBottom + 10;
  const hintBottom = hintTop + SIZE * 0.8;

  // 1-prstový PUSH-UP z hint zóny
  if (
    yCanvas >= hintTop &&
    yCanvas <= hintBottom &&
    dy < -25 &&
  ...
  // rozhodni, či posúvame riadok alebo stĺpec
  if (Math.abs(dx) > Math.abs(dy)) {
    // horizontálny swipe → posun RIADKU
    const dir = dx > 0 ? +1 : -1;
    shiftRow(row, dir);
  } else {
    // vertikálny swipe → posun STĹPCA
...
// ===== AURORA-STYLE POSUN RIADKOV/STĹPCOV =====
function shiftRow(r, dir) {
  // cyklický posun riadku
  const oldRow = grid[r].slice();
  const newRow = [];
  for (let c = 0; c < COLS; c++) {
    const from = (c - dir + COLS) % COLS;
    newRow[c] = oldRow[from];
  }
  grid[r] = newRow;
  startLineAnim("row", r, dir);
}

function shiftCol(c, dir) {
  const oldCol = [];
  for (let r = 0; r < ROWS; r++) oldCol.push(grid[r][c]);
  const newCol = [];
  for (let r = 0; r < ROWS; r++) {
    const from = (r - dir + ROWS) % ROWS;
    newCol[r] = oldCol[from];
  }
  for (let r = 0; r < ROWS; r++) grid[r][c] = newCol[r];
  startLineAnim("col", c, dir);
}
