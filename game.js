...
startBtn.onclick = () => {
  ...
  resize();
  initGrid();
  resolveAllMatches(); // vyčistí auto-matchy na štarte
...
// cluster-friendly generovanie rada
function generateClusteredRow(prevRowBelow) {
  ...
  for (let c = 0; c < COLS; c++) {
    ...
    let type;
    if (candidates.length > 0 && Math.random() < 0.7) { // silné clustre
...
function loop(timestamp) {
  ...
  const dt = 16; // stačí približne, zen hra nepotrebuje presné dt
...
canvas.addEventListener("pointerup", e => {
  ...
  handleSingleSwipe(dx, dy, pointerSwipeStart.x, pointerSwipeStart.y, "mouse");
...
canvas.addEventListener("touchmove", e => {
  ...
  // dvojprstový swipe hore = push-up (vždy povolené)
...
canvas.addEventListener("touchend", e => {
  ...
  if (e.touches.length === 0 && singleTouchStart) {
    ...
    handleSingleSwipe(dx, dy, singleTouchStart.x, singleTouchStart.y, "touch");
    singleTouchStart = null;
  }
...
function handleSingleSwipe(dx, dy, startX, startY, source) {
  if (Math.abs(dx) < 20 && Math.abs(dy) < 20) return;

  const canvasRect = canvas.getBoundingClientRect();
  const startYRelative = startY - canvasRect.top;

  const boardTop = offsetY;
  const boardBottom = offsetY + ROWS * SIZE;

  // Ak gesture začala POD boardom a smeruje hore → push-up
  if (startYRelative > boardBottom + 10 && dy < -30) {
    doPushUp();
    return;
  }

  // Ak gesture začala vnútri boardu → swap
  if (isInsideBoard(startX, startYRelative)) {
    const c = Math.floor((startX - offsetX) / SIZE);
    const r = Math.floor((startYRelative - offsetY) / SIZE);
    ...
    // gesture mimo boardu hore/dole ignorujeme
  }
}
