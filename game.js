...
function getBoardCoords(clientX, clientY) {
  ...
  return { inBoard, col, row, boardTop, boardBottom, yCanvas: y };
}
...
canvas.addEventListener("touchmove", e => {
  ...
  if (e.touches.length === 2 && multiTouchStart) {
    ...
    const startAvgY = (multiTouchStart[0].y + multiTouchStart[1].y) / 2;
    const currentAvgY = (t1.clientY + t2.clientY) / 2;
    const avgDy = currentAvgY - startAvgY;

    const boardInfo = getBoardCoords((multiTouchStart[0].x + multiTouchStart[1].x) / 2, startAvgY);
...
// ===== SINGLE SWIPE – SWAP / PUSH-UP =====
function handleSingleSwipe(dx, dy, startClientX, startClientY) {
  if (Math.abs(dx) < 15 && Math.abs(dy) < 15) return;

  const boardInfo = getBoardCoords(startClientX, startClientY);
  const { inBoard, col, row, boardBottom, yCanvas } = boardInfo;
