...
function generateClusteredRow(prevRowBelow) {
  ...
  for (let c = 0; c < COLS; c++) {
    ...
    let type;
    if (candidates.length > 0 && Math.random() < 0.8) { // ešte silnejšie clustre
...
// ===== PUSH ANIM (len jemný bump) =====
function startPushAnim() {
  pushAnim.active = true;
  pushAnim.t = 0;
  pushAnim.offset = -10; // mierne hore, potom späť
...
// ===== INPUT HELPERS =====
function isInsideBoardClient(clientX, clientY) {
...
canvas.addEventListener("touchmove", e => {
  ...
  // dvojprstový swipe hore = push-up (ľahký trigger)
...
function handleSingleSwipe(dx, dy, startX, startY) {
  ...
  // 1) Ak swipe začal POD boardom a smeruje hore → push-up
  if (startY > boardBottom + 10 && dy < -20) {
    doPushUp();
    return;
  }

  // 2) Ak swipe začal kdekoľvek (aj v boarde) a smeruje jasne hore → push-up
  if (dy < -40 && Math.abs(dy) > Math.abs(dx)) {
    doPushUp();
    return;
  }

  // 3) Swap – iba ak začal vnútri boardu
